...
async function getJobs({ page = 1, limit = 20, country, job_type, is_active, search } = {}) {
  ...
  return { total, page: Number(page), limit: Number(limit), pages: Math.ceil(total / limit), jobs: dataRes.rows };
}
...
async function updateJob(id, updates) {
  ...
  const res = await db.query(`UPDATE jobs SET ${setClauses} WHERE id = $${values.length} RETURNING *`, values);
  if (!res.rows[0]) { const e = new Error('Job not found'); e.status = 404; throw e; }
  return res.rows[0];
}
...
async function getScholarships({ page=1, limit=20, country, field, is_active, search }={}) {
  ...
  return { total, page: Number(page), limit: Number(limit), pages: Math.ceil(total / limit), scholarships: dataRes.rows };
}
...
async function updateScholarship(id, updates) {
  const allowed = ['title','provider','description','country','field','deadline','amount','apply_url','source_url','posted_at','is_active'];
  const fields = Object.keys(updates).filter(k => allowed.includes(k));
  if (!fields.length) { const e = new Error('No valid fields to update'); e.status = 400; throw e; }

  const setClauses = fields.map((f, i) => `${f} = $${i + 1}`).join(', ');
  const values = [...fields.map(f => updates[f]), id];
  const res = await db.query(`UPDATE scholarships SET ${setClauses} WHERE id = $${values.length} RETURNING *`, values);
  if (!res.rows[0]) { const e = new Error('Scholarship not found'); e.status = 404; throw e; }
  return res.rows[0];
}
