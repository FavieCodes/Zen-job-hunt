...
async function checkLimits(userId) {
  try {
    ...
    const { rows: totalRows } = await pool.query(
      `SELECT COUNT(*) AS cnt FROM interview_prep WHERE user_id = $1`,
      [userId]
    );
    ...
    logger.warn('[Interview] Limit check failed, allowing request: ' + err.message);
    return { allowed: true };
  }
}
...
module.exports = { generateInterviewPrep, getUserHistory, getPrepById, checkLimits, generateSingleAnswer };